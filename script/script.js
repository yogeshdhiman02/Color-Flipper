...
links.forEach(function(link){
    link.addEventListener('click',function(event){
        if(event.target.id === 'Simple'){
            buttons.forEach(nameBtnClicked);

        }
        else if(event.target.id === 'HexCode'){
            buttons.forEach(hexBtnClicked);
        }
    });
});


function nameBtnClicked(value){
...
function nameTextDisplay(){
    let randomColorName = colorNamesData[parseInt(Math.random() * colorNamesData.length)];
    mainSection.style.backgroundColor = `${randomColorName.hex}`;
    mainSection.style.transition = `background-color 0.3s ease-in-out`;
    hexCodeDisplayed.innerHTML = `${randomColorName.name}`;
}
